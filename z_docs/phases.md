...
*   **[NEW] Automated Evaluation & Groq Ragas Metric Scoring**: Migrated Ragas evaluation in `run_eval.py` to `ChatGroq(model="llama-3.1-8b-instant")` (500,000 TPD free tier quota), resolving 429 quota exhaustion errors and generating real benchmark metrics (**Faithfulness: 92.78%**, **Context Recall: 88.24%**, **Answer Relevancy: 76.70%**, **Context Precision: 76.64%**).
*   **[NEW] Knowledge Graph UI Overhaul & Bug Fixes**: Upgraded Tab 3 in `app.py` to display clean depth-scaled dot nodes (`shape: 'dot'`), multiple layout views (*Hierarchical Tree Top-Down*, *Hierarchical Tree Left-Right*, *Force-Directed Cluster*), heading depth filters (H1, H1+H2, All), visual color legend, and an interactive Concept Inspector card. Resolved 4 critical topology bugs:
    1. Enforced Min/Max Zoom boundaries (`0.4x` to `2.5x`) via Vis.js event listeners.
    2. Fixed Top-Down (`dag_ud`) and Left-Right (`dag_lr`) layout orientation mismatch by setting layout-specific `forceDirection` edge options (`horizontal` vs `vertical`).
    3. Restored directional edge arrow heads with bright stroke colors (`#64748B` default, `#00F0FF` highlight).
    4. Implemented default source document selection to main project docs and added a safety guard capping max rendered nodes at `250` (with user warning badge) to prevent DOM/Vis.js crashes when viewing 15,000+ nodes.